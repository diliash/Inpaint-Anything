...
FINAL_DIR = "/project/3dlg-hcvc/diorama"

def generate_html(gt_path, results_path, methods, selected_scenes, models_per_page=50):
    cur_dir = os.getcwd()
    # Get all model IDs
    model_ids = sorted([d for d in os.listdir(gt_path) if os.path.isdir(os.path.join(gt_path, d))])
    # Leave only selected scenes
    model_ids = [model_id for model_id in model_ids if model_id in selected_scenes]
    ...
    # subexp_dir,image_name,rmse,absrel
    dav1_gt_depth_metrics_path = f"{results_path}/results_depth_gt.csv"
    # subexp_dir,image_name,rmse,absrel
    dav2_gt_depth_metrics_path = f"{results_path}/results_depth_gt-v2.csv"
    ...
    dav1_gt_depth_metrics = {}
    with open(dav1_gt_depth_metrics_path, "r") as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            subexp_dir, image_name, rmse, absrel = row
            dav1_gt_depth_metrics[(subexp_dir, image_name)] = (rmse, absrel)
    
    dav2_gt_depth_metrics = {}
    with open(dav2_gt_depth_metrics_path, "r") as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            subexp_dir, image_name, rmse, absrel = row
            dav2_gt_depth_metrics[(subexp_dir, image_name)] = (rmse, absrel)
    ...
    for page in range(total_pages):
        ...
        html_content += "<tr><th>Model ID</th><th>GT</th><th>GT masked</th><th>GT Unfurnished</th>"
        for method in methods:
            html_content += f"<th>{method}</th>"
        html_content += "<th>GT Depth</th>"
        for method in methods:
            html_content += f"<th colspan='2'>{method} Depth</th>"
        html_content += f"<th colspan='2'>Full Scene Depth Masked</th>"
        html_content += "</tr>\n"
        # Separate subcolumns for Depth Anything v1 and v2
        html_content += "<tr><th></th><th></th><th></th><th></th><th></th>"
        for method in methods:
            html_content += "<th></th>"
        for method in methods:
            html_content += "<th>DAv1</th><th>DAv2</th>"
        for method in methods:
            html_content += "<th>Masked full DAv1</th><th>Masked full DAv2</th>"
        html_content += "</tr>\n"
        ...
        for model_id in page_model_ids:
            ...
            # GT masked image
            gt_masked_image_path = f"{results_path}/{method}/{model_id}/inpainted_merged_masked.png"
            html_content += f'<td><img src="{os.path.relpath(gt_masked_image_path, cur_dir)}" alt="GT Masked {model_id}"></td>'
            ...
            for method in methods:
                ...
                method_depth = next((f for f in os.listdir(method_depth_path) if f.endswith('depth-valid-color.png')), None)
                if method_depth:
                ...
                # DAv2
                method_depth_path = f"{results_path}/{method}/{model_id}"
                method_depth = next((f for f in os.listdir(method_depth_path) if f.endswith('depth-valid-color-v2.png')), None)
                if method_depth:
                    full_path = f"{method_depth_path}/{method_depth}"
            ...
            # Method Depths Masked
            for method in methods:
                # DAv1
                method_depth_path = f"{results_path}/{method}/{model_id}"
                method_depth = next((f for f in os.listdir(method_depth_path) if f.endswith('gt-valid-color.png')), None)
                if method_depth:
                    full_path = f"{method_depth_path}/{method_depth}"
                    html_content += f'<td><img src="{os.path.relpath(full_path, cur_dir)}" alt="{method} Depth {model_id}"></td>'
                else:
                ...
                # DAv2
                method_depth_path = f"{results_path}/{method}/{model_id}"
                method_depth = next((f for f in os.listdir(method_depth_path) if f.endswith('gt-valid-color-v2.png')), None)
                if method_depth:
                    full_path = f"{method_depth_path}/{method_depth}"
                    html_content += f'<td><img src="{os.path.relpath(full_path, cur_dir)}" alt="{method} Depth {model_id} v2"></td>'
                else:
            ...
            html_content += "<tr><td></td><td></td><td></td><td></td>"  # skip first columns
            for method in methods:
            ...
            for method in methods:
                subexp_dir = f"{results_path}/{method}"
                image_name = model_id
                rmse, absrel = dav1_gt_depth_metrics.get((subexp_dir, image_name), ("N/A", "N/A"))
                rmse_v2, absrel_v2 = dav2_gt_depth_metrics.get((subexp_dir, image_name), ("N/A", "N/A"))
                # Round to 3 decimal places
                rmse, absrel = map(lambda x: round(float(x), 3), (rmse, absrel))
                rmse_v2, absrel_v2 = map(lambda x: round(float(x), 3), (rmse_v2, absrel_v2))
                html_content += f"<td>RMSE: {rmse}<br>AbsRel: {absrel}</td><td>RMSE: {rmse_v2}<br>AbsRel: {absrel_v2}</td>"
    ...
    with open('inpatint_depth_eval_valid_gt_masked.html', 'w') as f:
        f.write(html_content)

    print(f"HTML file 'inpatint_depth_eval_valid.html' has been generated with {total_pages} pages.")

# Usage

gt_path = "/project/3dlg-hcvc/diorama/wss/scenes"
results_path = "/local-scratch2/diliash/diorama/third_party/Inpaint-Anything/results/gt-rerun-vis"
selected_scenes_path = "/project/3dlg-hcvc/diorama/wss/wss_scenes_selected.txt"
with open(selected_scenes_path, "r") as f:
    selected_scenes = f.read().splitlines()
methods = [path.split("/")[-1] for path in glob(f"{results_path}/*") if os.path.isdir(path)]
generate_html(gt_path, results_path, methods, selected_scenes)
generate_html(gt_path, results_path, methods, selected_scenes)
