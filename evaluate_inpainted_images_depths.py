...
if __name__ == "__main__":
    ...
    for subexp_dir in subexp_dirs:
        print(subexp_dir)
        inpainted_images = glob(f"{subexp_dir}/*/inpainted-scene.png")
    ...
    for subexp_dir in subexp_dirs:
        print(subexp_dir)
        inpainted_depths = glob(f"{subexp_dir}/*/inpainted-scene-depth.png")
        for inpainted_depth in inpainted_depths:
            ...
            inpainted_depth = cv2.imread(inpainted_depth, cv2.IMREAD_UNCHANGED) / 1000

            # Normalize depths
            gt_depth = (gt_depth - gt_depth.min()) / (gt_depth.max() - gt_depth.min())
            inpainted_depth = (inpainted_depth - inpainted_depth.min()) / (inpainted_depth.max() - inpainted_depth.min())

            rmse = np.sqrt(mean_squared_error(gt_depth, inpainted_depth))
...
if args.v2:
    ...
    for subexp_dir in subexp_dirs:
        print(subexp_dir)
        inpainted_depths = glob(f"{subexp_dir}/*/inpainted-scene-depth-v2.png")
        for inpainted_depth in inpainted_depths:
            scene_id = os.path.dirname(inpainted_depth).split("/")[-1]
            gt_depth = cv2.imread(f"{args.gt_path}/{scene_id}/depth.room.png", cv2.IMREAD_UNCHANGED) / 1000
            inpainted_depth = cv2.imread(inpainted_depth, cv2.IMREAD_UNCHANGED) / 1000
